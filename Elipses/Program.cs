...
namespace Ellipses
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[,] Edge = EdgeDetection.CannyEdge(Thresholded,ImageHeight, ImageWidth);
            
            Output.OutputImage(Edge, ImageHeight, ImageWidth, "Edge.tif", 255);
...
{
        /* fill hole algorithm */
        /* find contour edges - canny edge detection*/
        /* define contour edges - Topological structural analysis of digitized binary images by border following. Computer Vision, Graphics, and Image Processing, 30(1):32–46, 1985*/
        /* fill defined edges*/
}
class Threshold
{
...
class EdgeDetection
{
public static int[,] CannyEdge(int[,] SourceArray,int Height, int Width)
{

    int[,] OutputArray = Filter.GaussianBlur(SourceArray, Height, Width, 1.6);
    int[,] GradientIntensity, EdgeDirection;
    EdgeDetection.SobelEdge(OutputArray, Height, Width, out GradientIntensity, out EdgeDirection);
    OutputArray = NonMaximumSuppression(GradientIntensity, EdgeDirection, Height, Width);
    OutputArray = Threshold.DoubleThreshold(OutputArray, Height, Width, 0.1, 0.5);
    OutputArray = Hysteresis(OutputArray, Height, Width);
